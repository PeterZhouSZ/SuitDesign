...
class Config:
    def __init__(self):
        ...
        self.smallWhiteComponentsRemovement = False
        self.whiteComponentsSizeLowerBound = 1000
...
class UnitardDesignGenerator:
    ...
    def generateCodeSuit(self, templateFile ):
        ...
        for s in self.squares:
            if (not s.white):
                ...
                drawSquare4Triangles(self.suitWithChbMarginWithWhiteTriangle, s, self.ptsMask)
        ...
        if self._config.marginType == 'crop':
            suitFinal = Image.fromarray(self.suitWithChbMarginWithWhiteSquare)
        elif self._config.marginType == 'black':
            suitFinal = Image.fromarray(self.suitWithChb)
        elif self._config.marginType == 'whiteTriangle':
            suitFinal = Image.fromarray(self.suitWithChbMarginWithWhiteTriangle)

        if self._config.smallWhiteComponentsRemovement:
            self.removeSmallWhiteComponents()

        suitWithChbPIL = suitFinal
    ...
    def removeSmallWhiteComponents(self, img, color = 255):
        whiteComponents = ComponentExtractor.componentsExtractor(img, 255)
...
def drawSquare4Triangles(img, s):
...
def drawSquare4Triangles(img, s, ptsMask):
    midPts = np.empty((0,2), np.float64)
    cv2.rectangle(img, s.cornersInt[0], s.cornersInt[2], (0,), thickness=-1)
    for i in range(s.corners.shape[0]):
        nextI = (i + 1) % (s.corners.shape[0])
...
if __name__ == "__main__":
    ...
    uGenerator.writeCornerKeysSet(outCornerKeysFile)
